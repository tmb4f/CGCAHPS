...
IF OBJECT_ID('tempdb..#data_portal_master ') IS NOT NULL
DROP TABLE #data_portal_master
...
SELECT DISTINCT
	 ...
	 ,loc_master.epic_department_name_external
...
SELECT [key],
       ...
       mc_som_id,
       outpatient_flu,
       ambulatory_scorecard
INTO #data_portal_master
FROM DS_HSDM_App.Rptg.Data_Portal_Department_Master

--SELECT *
--FROM #data_portal_master
--ORDER BY level
--       , sid
...
SELECT dept.level,
       ...
       dept.name_external,
       dept.name,
       dept.service_line_id,
       CAST(service_line.sid AS INT) AS service_line_service_line_id,
       service_line.name_external AS service_line_name_external,
       service_line.name AS service_line_name
FROM #data_portal_master dept
LEFT OUTER JOIN
(
SELECT level,
       sid,
       ...
       name
FROM #data_portal_master
WHERE level = 'service_line'
)  service_line
ON CAST(service_line.sid AS INT) = dept.service_line_id
WHERE dept.level = 'clinic'
ORDER BY dept.service_line_id
       , CAST(dept.sid AS INT)
/*
--SELECT *
